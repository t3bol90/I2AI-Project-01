...
class GameController:
	...
	def StartGame(self, _level=1):
		...
		if(_level > 1):
			...
			for i in range(__totalGhost):
				self.ghost.append(self.graphic.DrawGhost(
					self.posGhost[i], i % __totalGhost))
				self.graphic.AnimateGhost(
					self.posGhost[i], self.posGhost[i], i % __totalGhost, self.ghost[i])
		self.foods = np.array(self.graphic.DrawFood(
		...
		if(self.isLose):
			text(self.graphic.to_screen((0, self.height/2)), formatColor(255.0/255.0, 0, 0),
			     "LOSE GAME", "Times", int(0.08*self.width*self.graphic.gridSize), "bold")
		else:
			_canvas_xs = (2+self.graphic.width)*self.graphic.gridSize
			_canvas_ys = (2+self.graphic.height) * \
                            self.graphic.gridSize + INFO_PANE_HEIGHT
			_color = formatColor(96.0/255.0, 172.0/255.0, 172.0/255.0)
			polygon([(0, 0), (0, _canvas_ys), (_canvas_xs, _canvas_ys), (_canvas_xs, 0)],
			        _color, fillColor=_color, filled=True, smoothed=False)
	...
	def Level3_4(self,_level):
		# Turn base
		visited_map = [[0] *self.width  for _ in range(self.height)]
		queue_food = Q.PriorityQueue()
		frequency = [[0] * self.width for _ in range(self.height)]

		while True:
			__next_move, __vision = self.PacmanTurn(queue_food, visited_map,frequency)
			if(__next_move is None):
				return
			self.AgentMove(__next_move, 0, True)
			# Draw Aura
			__auraImg = self.graphic.DrawAura(
				[self.ConvertIndexGraphic(i) for i in __vision])
			# Check is end game?
	...
	def GhostTurn(self, index, _level):
		...
		if(_level == 3):
			...
			travelsal, cost = astar_function(self.maze, self.ConvertIndexMaze(
				self.posGhost[index]), self.ConvertIndexMaze(self.posPacman), self.height, self.width)
	...
	def PacmanTurn(self, queue_food, visited_map,frequency):
		...
		if(len(ghost) > 0):
			...
			for i, j in __vision:
				visited_map[i][j] = 0
			__next_move, estimatedScore = cal_monster_with_minimax(
				self.maze, self.ConvertIndexMaze(self.posPacman), foods, ghost, frequency.copy())
			...
			if(not queue_food.empty() and __next_move == queue_food.queue[0][1]):
				...
				__next_move = cal_pos_nothing(
					self.maze, self.ConvertIndexMaze(self.posPacman), False, visited_map)
		else:
			# Case 1
			__next_move = cal_pos_nothing(
				self.maze, self.ConvertIndexMaze(self.posPacman), False, visited_map)
		...
		__vision, foods, ghost = get_vision(
			self.maze, __next_move, self.height, self.width)
	...
	def AgentMove(self, pos, index, isPacman=True):
		...
		if(isPacman):
			...
			self.graphic.AnimateGhost(
				self.posGhost[index], pos, index, self.ghost[index])
